...
6. Run `agents generate-plugin` (`bin/agents` at the repository root) to regenerate the `plugins/secure-cloud-agents/` pointer files for the new or changed role, and commit the result — the system-wide plugin is generated, not hand-maintained.
