...
`--root` defaults to the caller's working directory. Omit `--files` to use Git status in that target, including staged, unstaged, and untracked paths. Alternatively, use `--base <ref>` for committed `<ref>...HEAD` changes; that mode excludes dirty worktree changes. Non-Git targets require explicit `--files`. Review the emitted `inputs.repository_root` and `inputs.changed_files` before dispatch. `--output <path>` creates parent directories and overwrites the file, so use it only when run-artifact writes are authorized. Do not invent changed paths. Schema version 3 emits lifecycle `required_quality_gates` separately from mutation-oriented `human_gates`; attach both to each applicable brief. If the selector returns `needs-triage`, stop dispatch and request the missing scope. Validate every selected role against `agents/catalog.yaml`. When dispatching against a constrained-context (e.g. small local-model) runner, add `--context-profile local-small` to lower the default retrieval `--top` from 5 to 3 (an explicit `--top` still wins); see `agents/knowledge-store/README.md`'s "Running against a small-context local model" for the companion `cadre generate-plugin --context-profile compact` wrapper-side lever.
