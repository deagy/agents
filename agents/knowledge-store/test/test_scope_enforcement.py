...
class ScopeEnforcementTests(unittest.TestCase):
    ...
    def test_ac2_cadre_select_context_path_always_supplies_source(self) -> None:
        select_src = REPO_ROOT / "agents" / "orchestration" / "src"
        ...
        catalog = select_agents.load_catalog(select_agents.AGENTS_ROOT / "catalog.yaml")
    ...
    def test_ac12_agent_autonomy_knowledge_store_authority_unchanged(self) -> None:
        autonomy = yaml.safe_load((REPO_ROOT / "agents" / "shared" / "agent-autonomy.yaml").read_text(encoding="utf-8"))
