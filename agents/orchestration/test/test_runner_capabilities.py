...
REQ-CADRE-BACKLOG-8, `agents/orchestration/runs/cadre-idea-8-capability-manifest-2026-07-29/`).
...
MANIFEST_PATH = REPOSITORY_ROOT / "agents" / "runner-capabilities.json"
SCHEMA_PATH = REPOSITORY_ROOT / "agents" / "runner-capabilities.schema.json"
CATALOG_SCHEMA_PATH = REPOSITORY_ROOT / "agents" / "catalog.schema.json"
...
class ManifestExistenceAndContentTests(unittest.TestCase):
    ...
    def test_manifest_reproduces_catalog_schema_enums_without_hand_copying(self) -> None:
        """CM-FR-4 / gap G-3: agents/catalog.schema.json's capability/model/
...
class NarrativeContentUndisturbedTests(unittest.TestCase):
    ...
    def test_narrative_paragraphs_still_present(self) -> None:
        ...
        expected_markers = [
            ...
            "cline/cline#11435",
            "will go stale",
            "pip install -r agents/orchestration/mcp/requirements-mcp.txt",
        ]
...
class PackagingAllowlistParityTests(unittest.TestCase):
    ...
    def test_source_declares_both_files_in_the_allowlist(self) -> None:
        ...
        self.assertIn('"agents/runner-capabilities.json"', source)
        self.assertIn('"agents/runner-capabilities.schema.json"', source)
    ...
    def test_real_generator_packages_the_manifest_and_schema(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            (root / "agents").mkdir()
            (root / "agents" / "runner-capabilities.json").write_text(
                MANIFEST_PATH.read_text(encoding="utf-8"), encoding="utf-8"
            )
            (root / "agents" / "runner-capabilities.schema.json").write_text(
            ...
            self.assertTrue((plugin_root / "suite" / "agents" / "runner-capabilities.schema.json").is_file())
            packaged = json.loads(
                (plugin_root / "suite" / "agents" / "runner-capabilities.json").read_text(encoding="utf-8")
            )
    ...
    def test_scratch_removal_from_allowlist_breaks_the_packaged_copy(self) -> None:
        ...
        agents/runner-capabilities.json, and confirm the packaged output
        ...
        self.assertNotIn('"agents/runner-capabilities.json"', broken_source)
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            module_path = root / "agents" / "orchestration" / "src" / "generate_global_plugin.py"
