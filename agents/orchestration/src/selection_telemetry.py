...
1. Recording (`record_selection`) -- called from `select_agents.py`'s CLI
   ...
   *outcome* of selection (matched routes/risks, status, workflow, team
   ...
   governed solely by `agents/orchestration/selection.schema.json`.
...
- Local file only, never a network call. This module does not import
  ...
  `agents/orchestration/test/test_selection_telemetry.py`'s source-grep
...
- Records deliberately exclude the raw task text and raw changed-file paths.
  ...
  about; see `agents/knowledge-store/SECURITY.md`'s classification/handling
...
project-local-first convention (`agents/knowledge-store/src/config.py`)
...
Summarize accumulated telemetry:
    ...
    python3 agents/orchestration/src/selection_telemetry.py --summarize \\
