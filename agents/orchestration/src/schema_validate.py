#!/usr/bin/env python3
"""Strict, standalone JSON Schema validation for `agents/catalog.yaml` and
`agents/orchestration/routing.yaml`.
...
`agents/orchestration/selection.schema.json` precedent) hold the bulk of the
contract: `agents/catalog.schema.json` and
`agents/orchestration/routing.schema.json`. A small number of cross-field
...
DEFAULT_CATALOG_SCHEMA = DEFAULT_AGENTS_ROOT / "catalog.schema.json"
DEFAULT_ROUTING_SCHEMA = DEFAULT_AGENTS_ROOT / "orchestration" / "routing.schema.json"
...
def _find_missing_definitions(catalog: dict[str, Any], agents_root: Path) -> list[str]:
    """SV-FR-2: `definition` must resolve, relative to `agents/`, to a file
