...
- [plugins/agents/](plugins/agents/) packages this suite, its 39 roles, and the external `secure-cloud` provider profile.
...
If the target project actually uses this repository's own cloud stack (Proxmox, Talos, Kubernetes, Helm, OpenTofu, GitLab CI, PostgreSQL), use `--profile secure-cloud` instead of the default. This is the **recommended** way to get this repository's 39 roles into a project — scoped to that one project, generated once as static files the project owns from that point on (no live link back to this checkout, so a later role edit here doesn't silently change that project's behavior):
...
is for the narrower case of genuinely wanting all 39 roles, the 6 skills, and
