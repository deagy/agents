#!/usr/bin/env python3
"""Resolve a project-local overlay of `agents/orchestration/routing.yaml`.

Implements idea #6 (`agents/orchestration/runs/cadre-idea-6-routing-overlay-
...
`agents/shared/`'s `.agents/shared/<filename>` overlay (`agents/shared/src/
...
sections carry gating/review-separation semantics `agents/shared/`'s
...
same algorithm `agents/shared/src/resolve.py::find_project_overlay` and
`agents/knowledge-store/src/config.py` already use, reused here via
...
sys.path.insert(0, str(REPOSITORY_ROOT / "agents" / "shared" / "src"))

from resolve import deep_merge, find_file_at_project_root  # noqa: E402
from routing import load_routing  # noqa: E402

DEFAULT_ROUTING = REPOSITORY_ROOT / "agents" / "orchestration" / "routing.yaml"
...
# `agents/orchestration/routing.yaml`, a distinct artifact class from the
# `agents/shared/` policy-preference files. JSON-only (not YAML-or-JSON like
...
def _merge_knowledge_focus(base_kf: dict[str, Any], overlay_kf: Any) -> dict[str, Any]:
    ...
    `agents/shared/`'s ordinary structured-file merge rule.
    """
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description="Resolve a project-local overlay of agents/orchestration/routing.yaml",
    )
    parser.add_argument(
