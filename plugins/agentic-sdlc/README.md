...
A profile supplies candidate routing, artifact, gate, and validation defaults for a recognizable project shape. The current kernel includes `quick`, `generic`, `web-service`, and `secure-cloud`; `--profile auto` proposes one from observable repository files, favoring `quick` absent a stronger signal (it never proposes `secure-cloud` automatically — that one is opt-in only, since forcing cloud-specific roles onto a project with a different stack is exactly what this profile is designed to avoid). A human reviews the result. Check `init --help` because later releases may add profiles.
...
`secure-cloud` extends `generic` with the 14 additional roles from this repository's own 34-role catalog that aren't already in the kernel's generic set (infrastructure, pipeline, secrets/identity, database reliability, policy-as-code, supply-chain, cost/capacity, black-box/end-user testing, support escalation, documentation, and knowledge-store stewardship). It's the recommended way to get this repository's cloud-specific roles into a project, in place of the global `secure-cloud-agents` plugin install: unlike every other profile, it sets `rich_content_source: "secure-cloud-agents"` in its `profile.json`, which tells `write_codex_agent_wrappers()`/`write_claude_agent_wrappers()` in `scripts/agentic_sdlc.py` to bake the real `AGENT.md` text for each role into the generated wrapper (via `AGENT_CATALOG_SEARCH_PATH`, below) instead of the generic stub every other profile gets — a static, project-owned copy from the moment `init` runs, not a live link back to the source repository. Only `secure-cloud` opts into this; choosing any other profile is byte-for-byte unaffected even when `secure-cloud-agents` happens to be present alongside.

The portable kernel ships no extensions of its own — `extensions/` here is empty by design, keeping the kernel generic. `--extension <id>` resolves against `EXTENSIONS_SEARCH_PATH` in `scripts/agentic_sdlc.py`, which also checks a sibling `secure-cloud-agents/extensions/` directory when this repository's own domain plugin is present alongside it, without agentic-sdlc depending on it. This repository contributes one such extension, `sqs-platform` (defined at `plugins/secure-cloud-agents/extensions/sqs-platform/extension.json`), as an example: enable it during initialization with `--extension sqs-platform`. Projects that enable it receive the SQS category and specialized BOM applicability questions. Projects that do not use SQS retain the generic lifecycle; they should record the extension as not applicable with an owner and rationale when organizational policy requires that decision. Enabling an extension never supplies missing definitions: unresolved applicable terms remain `unknown` and block their affected gates. A project copying only `plugins/agentic-sdlc/` elsewhere can supply its own extensions by placing `extensions/<id>/extension.json` inside its own copy of the plugin.

The generic-vs-rich role content described above works the same way, via `AGENT_CATALOG_SEARCH_PATH`: the kernel's own `contracts/agent-catalog.json` (20 generic role stubs — just `{phase, kind}`, no real content) is checked first, then a sibling `secure-cloud-agents/agent-catalog.json` when present, which additionally carries a `definition` path to the real `AGENT.md` for all 34 roles. A project copying only `plugins/agentic-sdlc/` elsewhere has no sibling to find, so `secure-cloud` still resolves — just with the generic stub for every role, same as if `rich_content_source` weren't set at all.
