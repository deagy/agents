...
(see `requirements-mcp.txt`). Mirrors `agents/shared/src/resolve.py`'s
...
MCP_INSTALL_MESSAGE = (
    "The 'mcp' package is required to run the secure-cloud-agents MCP dispatch "
    "server; install it with `pip install -r "
    "agents/orchestration/mcp/requirements-mcp.txt` (stdio transport only -- "
...
def build_server():
    ...
    server = fast_mcp_cls("secure-cloud-agents-dispatch")
    ...
    def dispatch_secure_cloud_role(
        ...
        """Dispatch a named secure-cloud-agents role as a Codex CLI child process.

        role_id: catalog role identifier, e.g. "application-engineer"; must
            match ^[a-z0-9-]+$ and exist in agents/catalog.yaml.
