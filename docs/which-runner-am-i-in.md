...
| --- | --- | --- | --- |
| Generated per-role wrapper present | `plugins/cadre/agents/*.md` (or a project-local `.claude/agents/<role-id>.md` override) | `plugins/cadre/codex-agents/agents-*.toml`, synced to `~/.codex/agents/` | None — no generated wrapper exists for Cline yet |
...
| Subagent-dispatch tool name in the session | `Agent`/`Task` tool referencing a named subagent type | `spawn_agent` tool with a generic `agent_type` argument | Host-registered tools only; this repo's own Cline plugin (`plugins/cline/index.ts`) registers exactly one tool, `agents_select`, which plans only and never dispatches |
