...
The installed version comes from the plugin's own
`.claude-plugin/plugin.json`, not from the marketplace ref, and `release.yml`
only tags `main` from a state where all plugin manifests agree — so leave the
ref unpinned and use `/plugin update` to move forward. If your policy requires
a pinned source, append `@<tag>` from [the
releases](https://github.com/deagy/cadre-lifecycle/releases) and own keeping it
current; a stale pin is how you end up with a plugin whose
kernel-compatibility window no longer matches your installed kernel. A
*marketplace* source accepts a branch or tag but not a commit SHA, so the pin
is only as immutable as the tag itself. `owner/repo` shorthand clones over SSH
by default; set `CLAUDE_CODE_PLUGIN_PREFER_HTTPS=1` for HTTPS.
...
for this successor repository; install from a git checkout or the marketplace
add above instead:
...
catalog, and optional impact extensions to the Agentic SDLC kernel — the
supported kernel range is `provider.json`'s own `kernel_compatibility` block,
not this file. The repository
