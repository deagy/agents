...
- **Lifecycle governance** (Agentic SDLC, part of the optional `cadre-lifecycle-core`/`-github`/`-gitlab` plugins) provides G1–G10 gates for governed delivery. Gate state and transitions live entirely in the separately installed `agentic-sdlc` kernel (built from `kernel/` at the repository root) — role selection here does **not** execute gates; `bin/cadre select` calls `build_dispatch_plan.py` directly (no wrapper layer), and that script relays gate metadata the kernel already computed so a dispatch plan can be annotated with `required_quality_gates`/`human_gates`. Actual gate decisions are recorded through `bin/cadre sdlc` (a thin pass-through to the kernel), driven conversationally by the lifecycle plugins' skills.
...
AGENTS.md and README.md's "Regenerating Assets" cover the safe procedure. One caveat: this is currently a documentation-only safeguard. `roster/orchestration/src/generate_global_plugin.py`'s guard against overwriting an existing `--output` target only checks for the *presence* of a `.codex-plugin/plugin.json`, and `plugin/` has one (it is itself a packaged plugin) — so the guard does not actually stop `README.md` from being clobbered by a `--force-readme` run. Tracked at [deagy/cadre#97](https://github.com/deagy/cadre/issues/97). Once a structural guard lands, revisit whether this note is still needed.
