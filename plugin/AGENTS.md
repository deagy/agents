...
- **Cadre** (role selection, plugin `cadre`): 74 specialist roles, routing rules, orchestration runtime. The source of truth for role definitions is `roster/` at the repository root, in this same monorepo. Assets here (`agent-catalog.json`, `profiles/`, `extensions/`, `skills/`, `agents/`, `bin/cadre`, and `plugins/lifecycle/skills/`) are generated from it by `cadre generate-plugin --output plugin`.
- **Agentic SDLC** (lifecycle governance, plugins `cadre-lifecycle-core`/`-github`/`-gitlab`): G1–G10 gate schemas, run-record validation, gate authority. Owned by `kernel/` at the repository root — a permanently separate, independently versioned and released component (`kernel-v*` tags), installed as a standalone `agentic-sdlc` binary and invoked via `bin/cadre sdlc`, never vendored into this package. `plugins/lifecycle-github/` and `plugins/lifecycle-gitlab/` are entirely hand-authored here (including their own bundled onboarding/gate-review/pending-gates-briefing skill copies and kernel bootstrap script — hand-maintained duplicates of `cadre-lifecycle-core`'s generated equivalents, currently kept in sync by hand only — no test enforces it, so a change to the core copy must be propagated to both forge copies deliberately); `roster/` has no concept of them.
...
- **Cline Agents plugin** (`../cline-plugins/cline-agents/`): a distinct TypeScript plugin that ports the 74 Cadre catalog roles into Cline SDK agent presets a Cline session can dispatch as background subagents. Its `index.ts`/`package.json`/`test/`/`README.md` are hand-authored; `agents/*.md` (74 roles) and `skills/*.md` (8 skills) are generated from this repository's own `agents/*.md`/`skills/*/SKILL.md` via `tools/port_cline_agents.py` (run as `--root cline-plugins --source plugin`), wired into the same regeneration pipeline as everything else here.
