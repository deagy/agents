#!/usr/bin/env python3
"""Subcommand dispatcher for this repository's `agents` CLI.

bin/agents (POSIX sh) and bin/agents.ps1 (PowerShell) are thin, per-platform
...
Also runnable directly: `python bin/agents.py <subcommand> [args...]`.
"""
...
SDLC_INSTALL_MESSAGE = (
    "agents: Agentic SDLC v0.3.x is required; install it from https://github.com/deagy/agentic-sdlc"
)
...
def usage(subcommands: list[tuple[str, str, str]]) -> str:
    lines = ["Usage: agents <subcommand> [args...]", "", "Subcommands:"]
    ...
    lines.append("")
    lines.append("Each subcommand's own --help documents its arguments, e.g. `agents sdlc plan --help`.")
    return "\n".join(lines)
...
def dispatch_sdlc(rest: list[str]) -> int:
    ...
    if not sdlc_bin:
        ...
        return 1
    provider = REPO_ROOT / "plugins" / "secure-cloud-agents" / "provider.json"
...
def main(argv: list[str]) -> int:
    ...
    if match is None:
        print(f"agents: unknown subcommand '{command}'", file=sys.stderr)
