...
Use preferred Go libraries and tools from `../agents/shared/library-standards.yaml`. Dependencies and tool versions must be exact and reviewed. Format Go with `gofmt` and `goimports`; lint with `go tool golangci-lint run ./...` from `services/`. Add Gherkin scenarios for integration and regression behavior. Never commit `.env.local`, generated credentials, document fixtures containing sensitive data, database files, object-store contents, Terraform state, rendered secrets, or unpinned release artifacts.
